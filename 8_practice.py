...
# Очень часто студенты в своих проектах, исследованиях, диссертациях, курсовых и дипломных работах
# заменяют символы кириллицы на другие, похожие на символы русского алфавита, пытаясь спрятать
# следы использованных источников.
...
regex13 = r'(?:\b|\w+|\$)(?:[A-Za-z\d©ßé]+)\w+|(?:(?:[А-Яа-яёЁ]+)[A-Za-z]+)'


# Найдите все валидные пустые массивы, или массивы с числами. Числом считаем
# произвольную последовательность из цифр.
# Что нужно найти:
...
# После последнего числа может стоять запятая, т.к. такие массивы: [123, 123, ] и
# [23, ] валидные в Python

regex14 = r'(?:\[(\d+\,\s?)(\d+\,?\s?)*\])|\[\d+,?\s?\]|\[\]'
